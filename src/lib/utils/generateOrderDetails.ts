...
export const generateOrderDetails = (formData: OrderFormData): Order=>{
  ...
  const generatedName = formData.name || RANDOM_NAMES[Math.floor(Math.random() * RANDOM_NAMES.length)];
  const generatedQuantity = formData.quantity || DEFAULT_QUANTITIES[Math.floor(Math.random() * DEFAULT_QUANTITIES.length)];
